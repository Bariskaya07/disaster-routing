...
class SiameseEncoderDecoder(AbstractModel):
    def __init__(
        ...
        self.bottlenecks = nn.ModuleList(
            [
                self.bottleneck_type(self.filters[-idx - 2] * 2 + out_filters, out_filters)
        ...
        if shared:
            ...
            encoder_a = encoder_params[encoder_name]["init_op"]()
            encoder_b = encoder_params[encoder_name]["init_op"]()
            self.encoder_stages1 = nn.ModuleList(
                [self.get_encoder(encoder_a, idx) for idx in range(len(self.filters))]
            )
            self.encoder_stages2 = nn.ModuleList(
                [self.get_encoder(encoder_b, idx) for idx in range(len(self.filters))]
            )
        self._initialize_weights()

    def forward(self, input_x: torch.Tensor) -> torch.Tensor:
        encoder_outputs_pre: list[torch.Tensor] = []
        encoder_outputs_post: list[torch.Tensor] = []

        x = input_x[:, :3, ...]
        for stage in (self.encoder_stages if self.shared else self.encoder_stages1):
            x = stage(x)
            encoder_outputs_pre.append(torch.cat(x, dim=1) if isinstance(x, tuple) else x.clone())

        x = input_x[:, 3:, ...]
        for stage in (self.encoder_stages if self.shared else self.encoder_stages2):
            x = stage(x)
            encoder_outputs_post.append(torch.cat(x, dim=1) if isinstance(x, tuple) else x.clone())
        ...
        for idx, bottleneck in enumerate(self.bottlenecks):
            ...
            x = bottleneck(
                x,
                torch.cat(
                    [encoder_outputs_pre[reverse_idx - 1], encoder_outputs_post[reverse_idx - 1]],
                    dim=1,
                ),
            )
...
class DPNUnet(SiameseEncoderDecoder):
    def __init__(self, seg_classes: int, backbone_arch: str = "dpn92", num_channels: int = 3, shared: bool = True) -> None:
...
dpn_seamese_unet_shared = partial(DPNUnet, shared=True)

__all__ = ["DPNUnet", "dpn_seamese_unet_shared"]
